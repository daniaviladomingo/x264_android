...
#define YUV_UTILS_JAVA "daniel/avila/yuvutils/YuvUtils"
...
Jni_I420Scale(JNIEnv *env, jclass clazz, jbyteArray src, jint width, jint height, jbyteArray dst,
    ...
    jbyte *srcData = env->GetByteArrayElements(src, JNI_FALSE);
    jbyte *dstData = env->GetByteArrayElements(dst, JNI_FALSE);
    libyuv::I420Scale((const uint8 *) srcData, width,
                      (const uint8 *) (srcData + ySize), width >> 1,
                      (uint8 *) (srcData + ySize + (ySize >> 2)), width >> 1,
                      width, height,
                      (uint8 *) dstData, dst_width,
                      (uint8 *) (dstData + dstYSize + swap[swapUV]), dst_width >> 1,
